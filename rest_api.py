...
#4/2/2020 Added some V2 API commands
import requests
import os
...
class Account:
    ...
    def place_active_orderV2(self, side,  qty, price,stop_loss,take_profit,reduce_only,order_type="Limit",time_in_force='GoodTillCancel',close_on_trigger='True', trailing_stop=False):

        timestamp=int(time.time() * 1000)

        param_str = f"api_key={self.api_key}&close_on_trigger={close_on_trigger}&leverage={self.leverage}&order_type={order_type}&price={price}&qty={qty}&reduce_only={reduce_only}&side={side}&stop_loss={stop_loss}&symbol=BTCUSD&take_profit={take_profit}&time_in_force={time_in_force}&timestamp={timestamp}&trailing_stop={trailing_stop}"
        ...
        r=requests.post(self.url+'/v2/private/order/create',data)
    ...
    def place_stop_order(self, side,  qty, price,base_price,stop_px,trigger_by,order_type="Market",time_in_force='ImmediateOrCancel',close_on_trigger='True'):

        timestamp=int(time.time() * 1000)

        param_str = f"api_key={self.api_key}&base_price={base_price}&close_on_trigger={close_on_trigger}&leverage={self.leverage}&order_type={order_type}&price={price}&qty={qty}&side={side}&stop_px={stop_px}&symbol=BTCUSD&time_in_force={time_in_force}&timestamp={timestamp}&trigger_by={trigger_by}"
        ...
        logging.info('place_stop_order')

        r=requests.post(self.url+'/open-api/stop-order/create',data)
    ...
    def cancel_active_stop_order(self, stop_order_id):

        timestamp=int(time.time() * 1000)

        param_str = f"api_key={self.api_key}&leverage={self.leverage}&stop_order_id={stop_order_id}&symbol=BTCUSD&timestamp={timestamp}"
        ...
        logging.info('cancel_active_stop_order')

        r=requests.post(self.url+'/open-api/stop-order/cancel',data)
    ...
    def cancel_all_active_stop_order(self):
        ...
        r=requests.post(self.url+'/v2/private/stop-order/cancelAll',data)
    ...
    def my_position(self):
        ...
        #r=requests.get(self.url+'/v2/private/position/list',data)
    ...
    def get_orderbook(self):
        ...
        r=requests.get(self.url+'/v2/public/orderBook/L2',data)
    ...
    def replace_order_price(self, order_id, price):

        timestamp=int(time.time() * 1000)

        param_str = f"api_key={self.api_key}&leverage={self.leverage}&order_id={order_id}&p_r_price={price}&symbol=BTCUSD&timestamp={timestamp}"
    ...
    def replace_conditional_order(self, order_id, p_r_price, p_r_trigger_price):

        timestamp=int(time.time() * 1000)

        #param_str = f"api_key={self.api_key}&leverage={self.leverage}&order_id={order_id}&p_r_price={p_r_price}&p_r_trigger_price={p_r_trigger_price}&symbol=BTCUSD&timestamp={timestamp}"
        param_str = f"api_key={self.api_key}&leverage={self.leverage}&order_id={order_id}&p_r_trigger_price={p_r_trigger_price}&symbol=BTCUSD&timestamp={timestamp}"
        ...
        r=requests.post(self.url+'/open-api/stop-order/replace',data)
